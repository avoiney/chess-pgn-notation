...
const PAWN_MOVE_THEN_OTHER_MOVE = /^([a-h][1-8])([a-hBHNQR])$/;
const MOVE_REGEX =
  /^([a-h][3-6])|([a-h]x[a-h][1-8])|([a-h](1|8)=(B|H|N|Q|R))|((B|H|N|Q|R)[a-h][1-8])|((B|H|N|Q|R)x[a-h][1-8])|((B|H|N|Q|R)[a-h][a-h][1-8])|((B|H|N|Q|R)[a-h]x[a-h][1-8])$/;

export default function Controls({ moves, setMoves }) {
  const [color, setColor] = useState("white");
  const [turnMove, setTurnMove] = useState({ white: null, black: null });
  const [move, setMove] = useState("");
  const [error, setError] = useState(null);

  useEffect(() => {
    if (!move) return;
    if (move.match(PAWN_MOVE_THEN_OTHER_MOVE)) {
      const matchedMoves = move.match(PAWN_MOVE_THEN_OTHER_MOVE);
      const _turnMove = {};
      _turnMove[color] = matchedMoves[1];
      setTurnMove({ ...turnMove, ..._turnMove });
      setColor(color === "white" ? "black" : "white");
      setMove(matchedMoves[2]);
      setError(null);
    } else if (move.match(MOVE_REGEX)) {
      const _turnMove = {};
      _turnMove[color] = move;
      setTurnMove({ ...turnMove, ..._turnMove });
      setColor(color === "white" ? "black" : "white");
      setMove("");
      setError(null);
  ...
  useEffect(() => {
    if (turnMove.white && !turnMove.black) {
      setMoves([...moves, turnMove]);
    } else if (turnMove.white && turnMove.black) {
      moves[moves.length - 1] = turnMove;
      setMoves([...moves]);
      setTurnMove({ white: null, black: null });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [turnMove]);

  const updateMove = function (value) {
    setMove(move + value);
  };
  ...
  return (
    <React.Fragment>
      ...
      <button onClick={() => updateMove("B")}>B</button>
      <button onClick={() => updateMove("R")}>R</button>
      <button onClick={() => updateMove("H")}>H</button>
      <button onClick={() => updateMove("Q")}>Q</button>

      <button onClick={() => updateMove("a")}>a</button>
      <button onClick={() => updateMove("b")}>b</button>
      <button onClick={() => updateMove("c")}>c</button>
      <button onClick={() => updateMove("d")}>d</button>
      <button onClick={() => updateMove("e")}>e</button>
      <button onClick={() => updateMove("f")}>f</button>
      <button onClick={() => updateMove("g")}>g</button>
      <button onClick={() => updateMove("h")}>h</button>

      <button onClick={() => updateMove("1")}>1</button>
      <button onClick={() => updateMove("2")}>2</button>
      <button onClick={() => updateMove("3")}>3</button>
      <button onClick={() => updateMove("4")}>4</button>
      <button onClick={() => updateMove("5")}>5</button>
      <button onClick={() => updateMove("6")}>6</button>
      <button onClick={() => updateMove("7")}>7</button>
      <button onClick={() => updateMove("8")}>8</button>

      <button onClick={() => updateMove("x")}>x</button>
      <button onClick={() => updateMove("=")}>=</button>

      <button onClick={saveMove}>DONE</button>
